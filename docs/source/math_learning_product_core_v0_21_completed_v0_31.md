...
# COMPLETION ADDENDUM — v0.30 Build Closure

**Historical source:** Product Core v0.21-MA4 remains unchanged as the historical source of truth. This completion copy supersedes the open implementation boundaries only where explicitly stated below.

## Newly closed V1 contracts

- `MINI_GAME` is now a supported Experience Form.
- Game is an Experience mechanism, not a Learning Truth source.
- `QUEST` is an engagement/progression aid and never changes Learning Truth by itself.
- `REWARD` is a progression outcome and never implies Mastery.
- Daily Quests are limited to a maximum of 3 active child-facing goals.
- App open/login/time-only are not valid learning Quest triggers.
- Reward grants are idempotent.
- `ADVENTURE` remains a Mission-level composition; individual learning Evidence comes from its Encounters.
- Station 01 is the required first vertical slice.

## Revised Game Contract
...
Product Blueprint and high-level architecture remain closed. Content Contract `v0.28` and Session Runtime + Learning Rules `v0.29` are now specified in companion artifacts. This means the project is **ready for Station 01 vertical-slice implementation**, not for a claim that all six grades and all production content are complete.

## Remaining non-blocking / external gates

- educational approval of the provisional Grade 1 taxonomy;
- final child-privacy/legal review;
- full Grade 1 content authoring and QA;
- release hardening and store submission.

The canonical implementation sequence is:

`Core → DB → Auth/RLS → G1 Graph → Content v0.28 → Runtime v0.29 → Station 01 E2E → rollout.`

</div>


# COMPLETION ADDENDUM — v0.31 Parent Lite + Teacher Lite

## V1 Channel Decision

- Child Mobile remains the primary product experience.
- Parent Web is `PARENT LITE`: progress, insight, next step, recent learning, simple home support, settings.
- Teacher Web is `TEACHER LITE`: class, student inspection, needs-review/attention, simple objective, recheck, observation.
- Parent and Teacher are projections over shared Learning Truth; neither owns a parallel Learning Engine.
- Teacher orchestration remains bounded: Teacher chooses WHAT; Engine chooses HOW.

## Scope change

Teacher is not removed. Complex gradebook, assignment builder, group diagnosis, manual mastery editing, manual path editing and heavy analytics remain outside V1.

## Implementation order update

```text
Child Station 01 Core
      ↓
Parent Lite
      ↓
Teacher Lite
      ↓
Pilot / usability validation
      ↓
Selective expansion
```
