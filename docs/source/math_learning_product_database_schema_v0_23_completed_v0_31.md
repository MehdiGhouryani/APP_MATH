...
# COMPLETION ADDENDUM — v0.31 Parent Lite + Teacher Lite Data Boundary

## 1. Parent Lite persistence

No canonical Parent-specific learning tables are required in V1. Parent screens are projections over existing Evidence, Learning State, Decision/Plan, Session, Progression, Quest and Reward records.

## 2. Teacher Lite persistence

No full gradebook or assignment-builder schema is introduced in V1. A simple teacher objective can be represented through existing Decision / Learning Plan semantics with teacher provenance and bounded scope.

Teacher observations create provenance-bearing Evidence; they do not directly mutate Learning State or overwrite historical Evidence.

## 3. Authorization boundary

Parent → related child only.
Teacher → class students only.
Teacher → other class denied.
Admin → policy-based operational access.

These are projection/RLS boundaries, not UI-only rules.

## 4. Deferred schema

A dedicated Assignment / Gradebook aggregate remains deferred until the pilot demonstrates a real need that cannot be represented by existing Objective / Decision / Plan semantics.
