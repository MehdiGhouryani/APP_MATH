-- Phase 12 — transaction-capable PostgreSQL Learning Runtime boundary.
-- All authoritative runtime writes happen inside these SECURITY DEFINER functions.
-- The functions derive learner ownership from auth.uid() and never trust a client-supplied
-- learning_identity_id as an authority boundary.
...
begin
  ...
  if v_identity.id is null then
    raise exception using errcode = 'P0001', message = 'LEARNING_IDENTITY_NOT_FOUND';
  end if;
...
begin
  ...
  select ca.* into v_artifact from public.content_artifacts ca where ca.id = v_content.content_artifact_id and ca.status = 'ACTIVE';
  ...
  select sk.* into v_skill from public.skills sk where sk.id = v_artifact.skill_id and sk.code = p_skill_code and sk.status in ('ACTIVE','PROVISIONAL');
  ...
  insert into public.encounters(
    ...
    coalesce(p_learning_role, v_content.learning_role), coalesce(p_experience_form, v_content.experience_form), 'PRESENTED'
...
begin
  ...
  if v_encounter.learning_role='MASTERY_CHECK' and jsonb_array_length(p_answers) <> 5 then
  ...
  if v_existing.id is not null then
    ...
    select le.* into v_previous
    from public.learning_states le
    where le.learning_identity_id=v_identity.id and le.skill_id=v_encounter.skill_id and le.relationship_context_id=v_session.relationship_context_id;
    select scr.* into v_check
    from public.station_check_results scr
    where scr.attempt_id=v_existing.id;
    ...
    select count(*) into v_distinct_sessions
    from (select distinct session_id from public.station_check_results where learning_identity_id=v_identity.id and station_id=v_encounter.station_id and passed_check=true) q;
  ...
  v_expected := coalesce(v_content.evaluator_config->'expectedAnswers', '[]'::jsonb);
  ...
  for v_index in 0..greatest(v_answer_count,1)-1 loop
    if v_index < v_answer_count and v_index < v_expected_count then
  ...
  v_correct := v_expected_count > 0 and v_correct_count = v_expected_count and v_answer_count = v_expected_count;
  v_accepted := v_answer_count > 0;
  ...
  values (v_encounter.id, p_attempt_number, p_client_idempotency_key, v_evaluator_version, case when v_accepted then 'EVALUATED' else 'REJECTED' end, now(),
  ...
  if not v_accepted then raise exception using errcode='P0001', message='ANSWER_REJECTED'; end if;
  ...
  insert into public.learning_decisions(
    ...
    jsonb_build_array(v_evidence.id), 'phase12-postgres-v1', jsonb_build_object('step',v_selected_step),
    'temporary-4-of-5-in-two-checks-v2', 'SYSTEM', 'SELECTED', v_attempt.id
...
revoke all on function public.runtime_start_session(text,text,text,text,uuid,uuid) from public;
...
grant execute on function public.runtime_start_session(text,text,text,text,uuid,uuid) to authenticated;
...
comment on function public.runtime_submit_attempt(uuid,uuid,integer,text,jsonb) is 'Atomic server-authoritative Learning Runtime commit. Derives learner from auth.uid(), evaluates active content config, persists attempt/evidence/state/decision/plan/pass in one DB transaction.';
